#!/usr/bin/env python3
"""
This module combines multiple plots into one figure
"""
import numpy as np
import matplotlib.pyplot as plt


def all_in_one():
    """
    This function creates a figure with multiple subplots
    """
    y0 = np.arange(0, 11) ** 3

    mean = [69, 0]
    cov = [[15, 8], [8, 15]]
    np.random.seed(5)
    x1, y1 = np.random.multivariate_normal(mean, cov, 2000).T
    y1 += 180

    x2 = np.arange(0, 28651, 5730)
    r2 = np.log(0.5)
    t2 = 5730
    y2 = np.exp((r2 / t2) * x2)

    x3 = np.arange(0, 21000, 1000)
    r3 = np.log(0.5)
    t31 = 5730
    t32 = 1600
    y31 = np.exp((r3 / t31) * x3)
    y32 = np.exp((r3 / t32) * x3)

    np.random.seed(5)
    student_grades = np.random.normal(68, 15, 50)

    fig = plt.figure(figsize=(10, 8))
    fig.suptitle('All in One', fontsize='small')

    ax1 = plt.subplot2grid((3, 2), (0, 0))
    ax1.plot(y0)
    ax1.set_title('y = x³', fontsize='x-small')
    ax1.set_xlabel('x', fontsize='x-small')
    ax1.set_ylabel('y', fontsize='x-small')

    ax2 = plt.subplot2grid((3, 2), (0, 1))
    ax2.scatter(x1, y1, s=5)
    ax2.set_title('Random Scatter', fontsize='x-small')
    ax2.set_xlabel('x', fontsize='x-small')
    ax2.set_ylabel('y', fontsize='x-small')

    ax3 = plt.subplot2grid((3, 2), (1, 0))
    ax3.plot(x2, y2)
    ax3.set_title('Exponential Decay', fontsize='x-small')
    ax3.set_xlabel('Time', fontsize='x-small')
    ax3.set_ylabel('Fraction Remaining', fontsize='x-small')

    ax4 = plt.subplot2grid((3, 2), (1, 1))
    ax4.plot(x3, y31, label='5730')
    ax4.plot(x3, y32, label='1600')
    ax4.set_title('Decay Comparison', fontsize='x-small')
    ax4.set_xlabel('Time', fontsize='x-small')
    ax4.set_ylabel('Fraction Remaining', fontsize='x-small')
    ax4.legend(fontsize='x-small')

    ax5 = plt.subplot2grid((3, 2), (2, 0), colspan=2)
    ax5.hist(student_grades, bins=10)
    ax5.set_title('Student Grades Distribution', fontsize='x-small')
    ax5.set_xlabel('Grades', fontsize='x-small')
    ax5.set_ylabel('Number of Students', fontsize='x-small')

    plt.tight_layout()
    plt.show()
