...
# Шаг 2: A = σ(z)        (применяем сигмоид)
    ...
    def cost(self, Y, A):
        ...
        return cost
# кост тоже самое что лосс функция, только с другой терминологией. 
# Кост - это стоимость модели, которая показывает, насколько хорошо модель 
# предсказывает правильные метки. Чем меньше кост, тем лучше модель. 
# Лосс функция - это функция, которая используется для оптимизации модели
# во время обучения. Она измеряет разницу между предсказанными значениями и
# правильными метками. В данном случае, мы используем логистическую регрессию, 
# поэтому лосс функция - это логистическая функция потерь.